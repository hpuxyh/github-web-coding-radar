...
- 项目改造 MVP：先做一个可打开、可筛选、可复制提示词、可提交作品案例的静态版本。
...
- 每天精选适合改造的 GitHub 项目。
- 每个项目给小白解释、难度、坑点和可改造指数。
- 每个项目给 3 个改造方向、提示词和上线建议。
- 用户可以提交自己的改造成品，展示原项目、截图、链接和改造说明。

当前 MVP 已先把这条链路做成静态演示：

- 3 类高成功率项目：作品集落地页、数据看板、选择器小工具。
- 每类项目 3 个改造包：轻改造、进阶改造、想象方向。
- 每个改造包包含技术难度、小白难度、关键文件、不要先动的文件、运行命令、部署建议和提示词。
- 作品提交先用浏览器本地存储演示，后续可接 GitHub Issue、表单服务或数据库。
